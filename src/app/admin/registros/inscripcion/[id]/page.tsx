...
export default function RegistroDetailPage() {
  ...
  return (
    <div>
      ...
      {/* Inscribir en otro curso */}
      <div className="bg-white rounded-lg border border-gray-200 p-6 mb-6">
        <h2 className="font-semibold text-gray-800 mb-1">Inscribir en otro curso</h2>
        <p className="text-xs text-gray-500 mb-4">
          Los datos del alumno se transferirán automáticamente al formulario de inscripción. Solo deberás seleccionar el curso destino.
        </p>
        <button
          onClick={() => {
            const prefill = {
            ...
            try {
              sessionStorage.setItem("prefillStudent", JSON.stringify(prefill));
              router.push("/admin/inscripcion");
            } catch {}
          }}
          className="bg-[#0072CE] hover:bg-[#005fa3] text-white px-5 py-2 rounded-lg text-sm font-medium transition inline-flex items-center gap-2"
        >
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
