-- Refresh embedded HTML for the 7 module lessons of ENAE/UAS/M4
-- (strips intra-course nav + footer that conflict with the LMS)

UPDATE module_activities ma
...
'
FROM course_modules cm, courses c
WHERE ma.module_id = cm.id
  AND cm.course_id = c.id
  AND c.code = 'ENAE/UAS/M4'
  AND cm.title LIKE 'Módulo 1:%'
  AND ma.type = 'html';
...
WHERE c.code = 'ENAE/UAS/M4'
  AND ma.type = 'html'
  AND ma.description NOT LIKE '%nav-modulos%'
  AND ma.description NOT LIKE '%pie-curso%';